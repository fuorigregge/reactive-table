...
Template.reactiveTable.helpers({
    ...
    'getField': function (object) {
        //console.log(this, Template.instance().data.settings.flipColumn);
        //console.log(Template.parentData(2).templateData.settings.flipColumn);
        var flipColumn;
        //todo apply css dynamically
        if(Template.parentData(2).templateData.settings.flipColumn !== false){
            flipColumn = Session.get('isMobileMq');
        }

        var fn = this.fn || function (value) { return value; };
        var key = this.key;
        var value = get(object, key);
        var field =  fn(value, object);
        if(flipColumn) {
            var html = '<div style="font-weight: bold;width:50%;float:left;" >' + this.label + '</div>';
            if(field) {
                html +=field;
            } else {
...
Template.reactiveTable.onRendered(function(){
    var isMobileMq = !$('.reactive-table thead > tr').is(':visible');
    Session.set("isMobileMq", isMobileMq);
});
